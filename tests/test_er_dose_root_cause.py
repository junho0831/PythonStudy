...
from er_dose.root_cause import EuvRootCauseParser
...
def test_parse_euv_root_cause_contents():
    parsed = EuvRootCauseParser().parse(SAMPLE_EUV_CONTENTS)
...
def test_non_root_cause_contents_are_skipped():
    assert EuvRootCauseParser().parse("system info: normal message") is None
