...
`ER_DOSE_EUV`는 `mbeat.er_data_raw_euv` 기반 root cause 결과용 실행입니다. 대상 결과는 `prism_common.er_dose_euv_parsed`에 저장하며, `contents`에서 `source_file_name`, `source_exposure_id`, `source_code_occur_time`, `root_cause_code`, `root_cause_message`와 각종 EUV metric 컬럼을 파싱해 적재합니다. 실제 원천 데이터 호환을 위해 `Root cause`와 `Root clause`, `Exposure ID`와 `Exposesue I D`를 각각 동일한 라벨로 처리합니다. 숫자 뒤의 단위는 제외하고 저장하며, 개별 필드 변환 실패 시 해당 컬럼만 `NULL`로 처리합니다.
