...
`ER_DOSE_RAW`의 환경변수 기반 기본 실행은 최근 4일 lookback 모드입니다. 실행일 기준 `오늘 포함 최근 4일`을 날짜별로 검사하고, 각 날짜에 대해 원천 raw 건수와 적재 대상 parsed 건수를 비교합니다. 건수가 같으면 해당 날짜는 스킵하고, 다르면 그 날짜 파티션만 `TRUNCATE` 후 하루치 raw를 다시 파싱해 적재합니다. 반면 `ER_DOSE_RAW_TARGET_DATE` 또는 `ER_DOSE_START_TIME`/`ER_DOSE_END_TIME`를 명시하면 lookback 대신 지정한 날짜 또는 범위만 처리합니다. `ER_DOSE_EUV`는 이 lookback/count 비교 전략을 사용하지 않고, 지정한 날짜 또는 시간 범위를 그대로 처리합니다.
