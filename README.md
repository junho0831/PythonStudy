...
배치는 `code_occur_time` 기간 조건으로 조회한 후보를 한 번에 메모리로 올리지 않고, `chunk` 단위로 읽어서 파싱 후 바로 `COPY` 적재합니다. 기본 `chunk` 크기는 `ER_DOSE_RAW` 배치의 경우 `1000` (Airflow OOM 방지를 위해 하향 조정됨), `ER_DOSE_EUV` 배치의 경우 `10000`이며 실행 시 조정할 수 있습니다. 또한, DB 조회 시 SQLAlchemy 서버사이드 커서(`stream_results=True`, `max_row_buffer=chunk_size`)를 활성화하여 드라이버 단의 메모리 병목을 차단합니다. 청크 단위로 처리되더라도 설비(`eq_name`)별로 이전에 파싱한 `wafer_id`와 `wafer_seq`를 기억하여, 해당 값이 없는 로그에 이전 값을 채워넣는 로직이 적용되어 있습니다.
