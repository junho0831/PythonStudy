...
RAW 마지막 단계에서 RAW/EUV 통계를 각각 조회해 `mbeat.batch_event_log`에 배치별 한 행씩 저장합니다. 각 행의 `data.equipment_counts` 배열에 설비별 원천/parsed 건수를 담습니다. 로그 테이블이 없으면 [create_batch_event_log.sql](er_dose/sql/create_batch_event_log.sql)을 먼저 적용해야 합니다.
...
EUV 적재가 RAW 마지막 집계 전에 완료되는 운영 순서를 전제로 합니다. RAW 마지막에서 DELETE 메서드 두 개를 직접 실행한 뒤 서머리 INSERT 두 개와 RAW/EUV 통계 작업 두 개를 먼저 모두 제출하고 마지막에 완료를 기다립니다. DELETE와 INSERT는 별도 실행하며 하나의 트랜잭션으로 묶지 않습니다. 통계 작업은 각각 조회 후 INSERT하며 RAW와 EUV를 별도 행으로 저장합니다. 각 행은 `batch_name`으로 구분하고, `data`에는 `equipment_counts` 배열을 넣습니다. 배열 원소는 `eq_name`, `source_count`, `target_count`이며 해당 배치의 모든 설비를 담습니다. 설비별 DB 행이나 eq_name 컬럼은 추가하지 않습니다. 처리 시간 범위는 `message`, 시작일은 `target_date`, 저장 시각은 `created_at`에 기록합니다. 통계는 다른 병렬 작업이 실패해도 먼저 저장될 수 있으므로 전체 배치 성공을 의미하지 않습니다. 기존 리로드 판단은 유지합니다.
...
설비 건수 로그의 메시지와 JSON 형식은 RAW processor의 `_write_equipment_count_log()`에서 구성합니다. 공통 `batch_log_repository.insert_batch_log()`는 호출자가 전달한 배치명·이벤트·메시지·데이터를 직렬화해 저장하며, 특정 배치의 메시지나 필드를 추가하지 않습니다.
