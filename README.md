...
배치는 `code_occur_time` 기간 조건으로 조회한 후보를 한 번에 메모리로 올리지 않고, `chunk` 단위로 읽어서 파싱 후 바로 `COPY` 적재합니다. 기본 `chunk` 크기는 `ER_DOSE_RAW` 및 `ER_DOSE_EUV` 배치 모두 `30000`이며 실행 시 조정할 수 있습니다. `stream_results=True`와 `max_row_buffer` 옵션이 켜져 있으므로 청크 크기를 늘려도 메모리가 안전하게 제어되며 처리 속도가 대폭 향상됩니다. 청크 단위로 처리되더라도 설비(`eq_name`)별로 이전에 파싱한 `wafer_id`와 `wafer_seq`를 기억하여, 해당 값이 없는 로그에 이전 값을 채워넣는 로직이 적용되어 있습니다.
