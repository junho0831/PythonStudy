...
이 문서는 reticle backside 오염 분석을 위해 `RUBI` 텍스트 검사 결과와 `RUIP` 이미지를 시간 기준으로 매칭하는 규칙을 설명합니다. 매칭된 데이터는 RBI fail 원인 particle과 관련 이미지를 함께 추적하기 위한 분석 입력으로 사용됩니다.
...
이 방식은 이미지 자체보다 RBI 검사 텍스트를 분석 기준 데이터로 보고, 이미지를 해당 검사 결과의 근거 자료로 연결하기 위한 선택입니다. 이미지 기준 처리나 수동 매칭도 가능하지만, 현재 데이터 구조에서는 파일명 prefix와 timestamp를 이용한 텍스트 기준 자동 매칭이 가장 일관된 처리 방식입니다.
